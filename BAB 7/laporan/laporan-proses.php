...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mysqli_query($conn, $queryLaporan)) {
        ...
        header('Location: laporan.php?status=success&message=' . urlencode('Data berhasil ditambahkan!'));
        exit;
    } else {
        header('Location: laporan.php?status=error&message=' . urlencode('Terjadi kesalahan saat menyimpan data.'));
        exit;
    }
}
?>
